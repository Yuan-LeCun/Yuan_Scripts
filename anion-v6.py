...
def main():
    start = time.time()
    boxsize = 51.83015687378338
    atoms = 12428
    with open('file_list.txt', 'r') as f:
        csv_files = [file.strip() for file in f]
    ...
    # 收集所有进程的结果
    all_results = list(pool.imap(process_file_FSI, args))
    
    # 合并所有字典结果
    final_ratioFSI = {}
    # 添加进度提示
    total_files = len(csv_files)
    for i, result in enumerate(pool.imap(process_file_FSI, args)):
        print(f'\rProcessing: {i+1}/{total_files}', end='')
        for key, value in result.items():
            final_ratioFSI[key] = final_ratioFSI.get(key, 0) + value
    print()  # 换行
    ...
    # 写入文件
    # ratioFSI从大到小排序
    final_ratioFSI = dict(sorted(final_ratioFSI.items(), key=lambda x: x[1], reverse=True))
    with open('ratioFSI2.txt', 'w') as f:
        for key, value in final_ratioFSI.items():
            f.write(f'{key}: {value}\n')
    print(final_ratioFSI)
