...
echo "Base_Name,ESPmin,ESPmax,MPI,HOMO,LUMO,Energy" > "$output_file"
...
EOF
    then
        ...
        Energy=$(grep "Total energy" out.txt | awk '{print $3}')
    else
        echo "Error processing ${base_name}.fchk for HOMO/LUMO/Energy" >> "$output_file"
        continue
    fi

    # Write minimal, maximal, MPI, HOMO, and LUMO to the output file only if base_name contains "energy"
    
    echo "$base_name $minimal $maximal $MPI $HOMO $LUMO $Energy" >> "$output_file"
    
    # echo "$base_name $minimal $maximal $MPI $HOMO $LUMO" >> "$output_file"
    rm -f out.txt
done
